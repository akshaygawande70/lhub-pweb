					...
					<c:choose>
						<c:when test="<%=user2 == null%>">
							...
							<div class="box-input">
								...
								<aui:input name="login" size="30" label="" showRequiredLabel=""
									cssClass="clearable cs-form" type="text" value="<%=login%>"
								>
							...
							<div class="abstrak" style="font-size:13px">Please key in the email address which you have used to create the account.</div>
							<aui:button-row>
								<button type="submit" class="btn-grey w100"><liferay-ui:message key="send-new-password" /></button>
							</aui:button-row>
						</c:when>
						<c:when test="<%=user2 != null%>">
...
<script type="text/javascript"> 
	$(document).ready(function() {
		$("#<portlet:namespace/>login").on("change", function(){
			var redirectUrl = $("#<portlet:namespace/>redirect").val();
			//redirectUrl += "&<portlet:namespace/>email="+encodeURI($(this).val());
			$("#<portlet:namespace/>redirect").val(redirectUrl);
		});
	});
</script>