<%@page import="web.ntuc.nlh.eventfilter.config.EventFilterConfig" %>
...
<aui:form action="${configActionURL}" method="post" name="fm">
	<liferay-ui:panel-container extended="true" id="scope">
		...
		<liferay-ui:panel title="" collapsible="false" extended="true" defaultState="expanded">
			<aui:fieldset label="">
				<div class="row">
					<aui:select name="eventType" value="${eventType}"
						label="Event Type">
						<aui:option value="0">Select Event Type</aui:option>
						<c:forEach items="${eventType}" var="e">
							<aui:option value="${e.typeId}">${e.typeName}</aui:option>
						</c:forEach>
					</aui:select>
				</div>
			</aui:fieldset>
