...
<%@page import="web.ntuc.eshop.registeredcourse.dto.ExamResultDto"%>
<%@page
	import="web.ntuc.eshop.registeredcourse.constants.MVCCommandNames"%>
<%@ include file="../init.jsp"%>

<%
	List<ExamResultDto> examResultListObj = (List<ExamResultDto>) request.getAttribute("examResultList");
	JSONArray examResultListJson = JSONFactoryUtil.createJSONArray(examResultListObj);
%>
<portlet:resourceURL
	id="<%=MVCCommandNames.COURSE_EXAM_RESULT_DETAIL_RESOURCE%>"
	var="examResultDetailURL">
	...
	<portlet:param name="examName" value="${examName}" />
</portlet:resourceURL>
	...
	<div class="container">
		<div class="row">
			<div class="box-name">
				...
				<div class="email">Your exam result.</div>
			</div>
			<hr class="hr-bold" />
		</div>
	</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		getImgStr();
		var examResultTblId = "#<portlet:namespace/>examresult";
		var examResultDataTable = $(examResultTblId).dataTable({
			"bFilter" : false,
			"responsive" : true,
			"bServerSide" : true,
			"bAutowidth" : true,
			...
			"sAjaxSource" : '${examResultDetailURL}',
			"aoColumnDefs" : [ {
				"mDataProp" : "exam_name",
				"bSortable" : false,
				"aTargets" : [ 0 ]
	...
	function downloadPdf() {
		...
		var examResultList = <%=examResultListJson %>;
		
		var rows = [];
		rows.push([ 'Exam Name', 'Exam Date', 'Exam Results' ]);
		examResultList.forEach(function(data){
			rows.push([ data.examName, data.examDate, data.examResult ]);
		});
		try {
			var dd = {
				footer : {
