...
@Component(immediate = true, property = { "mvc.command.name=" + MVCCommandNames.SEARCH_NEWS_DATA_RESOURCES,
		"javax.portlet.name=" + SearchResultPortletKeys.SEARCH_RESULT_PORTLET }, service = MVCResourceCommand.class)
public class SearchNewsResultDataResource implements MVCResourceCommand {
	private static Log log = LogFactoryUtil.getLog(SearchNewsResultDataResource.class);
...
//			searchContext.setAttribute("paginationType", "more");
			...
			List<String> ddmStructureKeys = CommonSearchResult.getDDMStrunctureKey(SearchResultPortletKeys.NEWS_CATEGORY_NAME);
			...
			int resCount = common.getDtoResultCount(globalGroupId, localGroupId,
					SearchResultPortletKeys.NEWS_CATEGORY_NAME, searchContext, themeDisplay, request,
					resourceRequest, resourceResponse, course);
			searchContext.setStart(showFrom);
			searchContext.setEnd(showTo);
			
			List<SearchResultDto> newsResult = common.getDtoResult(globalGroupId, localGroupId,
					SearchResultPortletKeys.NEWS_CATEGORY_NAME, searchContext, themeDisplay, request,
					resourceRequest, resourceResponse, course);
