...
@Component(immediate = true, property = { "mvc.command.name=" + MVCCommandNames.SEARCH_PRESS_DATA_RESOURCES,
		"javax.portlet.name=" + SearchResultPortletKeys.SEARCH_RESULT_PORTLET }, service = MVCResourceCommand.class)
public class SearchPressResultDataResource implements MVCResourceCommand {
	private static Log log = LogFactoryUtil.getLog(SearchPressResultDataResource.class);
...
//			searchContext.setAttribute("paginationType", "more");
			...
			List<String> ddmStructureKeys = CommonSearchResult.getDDMStrunctureKey(SearchResultPortletKeys.PRESS_CATEGORY_NAME);
			...
			int resCount = common.getDtoResultCount(globalGroupId, localGroupId,
					SearchResultPortletKeys.PRESS_CATEGORY_NAME, searchContext, themeDisplay, request,
					resourceRequest, resourceResponse, course);
			searchContext.setStart(showFrom);
			searchContext.setEnd(showTo);
			List<SearchResultDto> pressReleasesResult = common.getDtoResult(globalGroupId, localGroupId,
					SearchResultPortletKeys.PRESS_CATEGORY_NAME, searchContext, themeDisplay, request,
					resourceRequest, resourceResponse, course);
