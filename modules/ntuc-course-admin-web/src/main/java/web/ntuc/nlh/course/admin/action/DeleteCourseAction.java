...
@Component(immediate = true, property = { "mvc.command.name=" + MVCCommandNames.DELETE_COURSE_ACTION,
		"javax.portlet.name=" + CourseAdminWebPortletKeys.COURSE_ADMIN_PORTLET }, service = MVCActionCommand.class)
public class DeleteCourseAction extends BaseMVCActionCommand {
	private static Log log = LogFactoryUtil.getLog(DeleteCourseAction.class);

	@Override
	protected void doProcessAction(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		log.info("Delete course action - start");
		try {
			...
			log.error("Error while delete course: " + e.getMessage(), e);
		}
		log.info("Delete course action - end");
	}
}